...
Há **duas** páginas de demonstração na raiz do repositório: **`index.html`** — paradigma verbal (infinitivo → tabelas, estilo verbe.cc), só `conjugai-core.js`; e **`demo/caa/index.html`** — frases telegráficas com passos e `analisarFrase`, com `app.js` + `conjugai-core.js`. A interface de **análise ao vivo** (quatro passos) é esta segunda; serve para **inspecionar** o pipeline do motor em contexto **CAA**.
