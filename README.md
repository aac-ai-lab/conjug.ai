...
Há **duas** páginas de demonstração na raiz do repositório: **`index.html`** — Demo infinitivo em português e tabelas geradas pelo `conjugai-core` (tempos simples e compostos, incluindo linha de `vós`), só `conjugai-core.js`; e **`demo/caa/index.html`** — frases telegráficas com passos e `analisarFrase` (inclui **orações compostas coordenadas** quando a segmentação é possível), com `app.js` + `conjugai-core.js`. A interface de **análise ao vivo** (quatro passos) é esta segunda; permite **validar** a correção e o pipeline do motor em contexto **CAA**. Na barra lateral, cada exemplo mostra **badges** (sujeito, tempo, conjugação, etc.) como guia explicativo — mostram o que o motor identificou internamente.
