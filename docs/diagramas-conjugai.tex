...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[node distance=6mm, auto]
    ...
    \node[proc, below right=8mm and 4mm of c] (s0) {\texttt{segmentar orações}\\\footnotesize\textit{(coordenação)}};
    \node[proc, below=of s0] (d) {\texttt{detectarSujeito}};
    ...
    \draw[->] (c) -| node[near start, right, font=\scriptsize] {não} (s0);
    \draw[->] (s0) -- (d);
    \draw[->] (d) -- (e);
    \draw[->] (e) -- (f);
    \draw[->] (f) -- (g);
  ...
  \caption{Fluxo principal desde o texto bruto até \texttt{correcao} (\texttt{vendors/conjugai-core/index.ts}). Com mais de uma oração coordenada, os passos após a segmentação repetem-se por oração antes de juntar correções.}
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[node distance=5mm]
    ...
    \node[proc, below=of x, text width=0.92\textwidth, align=left, font=\small] (steps) {%
      \textbf{Ordem em} \texttt{conjugador.ts}\textbf{:}\\[0.3em]
      1) Se o 1.\textsuperscript{o} token for presente de \textit{ir} e existir infinitivo à frente $\to$ lema \texttt{ir}.\\
      2) Se \texttt{detectarLocucaoVerbalHeadLemma} casar (\textit{ter que}/\textit{ter de}, \textit{poder}/\textit{dever} + inf., \ldots) $\to$ lema da locução.\\
      3) Último \textit{que} que não seja \textit{ter que} $\to$ primeiro infinitivo à direita (subordinação).\\
      4) Senão, primeiro token com forma \texttt{-ar/-er/-ir/-pôr} na ordem dos tokens.\\
      5) \texttt{detectarVerboPorDicionario} (filtro) ou \texttt{null}.};
    \draw[->] (x) -- (steps);
  \end{tikzpicture}
  \caption{Identificação do lema verbal (\texttt{conjugador.ts}); alinhado a \texttt{docs/diagrama.md} e \texttt{diagrama.html}.}
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[node distance=7mm]
    \node[dec] (sc) {Prefixo \emph{X e Y} ou \emph{X ou Y}\\antes do verbo?};
