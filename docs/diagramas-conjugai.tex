...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[node distance=5mm]
    ...
    \node[proc, below=of x, text width=0.92\textwidth, align=left, font=\small] (steps) {%
      ...
      2) Se \texttt{detectarLocucaoVerbalHeadLemma} casar (\textit{ter que}/\textit{ter de}, \textit{poder}/\textit{dever} + inf., \textit{estar a}, \textit{começar a}, \ldots) $\to$ lema da locução.\\
      3) Senão, se existir token com forma de infinitivo \texttt{-ar/-er/-ir/-pôr} $\to$ o primeiro desses.\\
      4) \texttt{detectarVerboPorDicionario} (filtro de pronomes/partículas) ou \texttt{null}.};
    \draw[->] (x) -- (steps);
  \end{tikzpicture}
